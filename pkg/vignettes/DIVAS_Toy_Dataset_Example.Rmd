...
First, ensure you have the `DIVAS` package and its dependencies installed. See the main `README.md` for installation instructions. This vignette follows the current 1.x `CVXR` line (tested with `CVXR` 1.0-15).
...
# Keep the toy example on defaults for centring:
#   colCent = FALSE, rowCent = FALSE
# Note: ReturnDetail = TRUE is needed if you plan to run
# DJIVEAngleDiagnosticJP() afterwards.
# divas_results <- DIVAS::DIVASmain(
...
For this toy dataset, we keep `rowCent = FALSE` (default). The toy matrices were generated as a benchmark scenario where the default preprocessing reproduces the expected joint/partially shared component pattern. In contrast, for many real-world omics analyses (e.g., COVID case study), `rowCent = TRUE` is often preferred to remove feature-wise baselines and focus on between-sample variation.
